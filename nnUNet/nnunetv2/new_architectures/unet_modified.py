...
class SEBlock3D(nn.Module):
    ...
    def forward(self, x: torch.Tensor, g: torch.Tensor = None):
        ...
        if c != self.channels:
            print(
                f"Input tensor x has {c} channels, but initialized with {self.channels} channels")
            assert c == self.channels, "Channel mismatch in SEBlock"
        ...
        if g is not None:
            ...
            if x.shape[1] != g_aligned.shape[1]:
                print(
                    f"Channels mismatch ({x.shape[1]}) and g ({g_aligned.shape[1]})")
            else:
...
class ASPP(nn.Module):
    def __init__(self,
        ...
        # Fuse
        total = out_channels * (2 + len(dilation_rates))
...
class PlainConvEncoder_modified(nn.Module):
    def __init__(self,
                 input_channels: int,
                 n_stages: int,
...
class UNetDecoder_modified(nn.Module):
    def __init__(self,
                 encoder: PlainConvEncoder_modified,
                 num_classes: int,
        ...
        for i in range(n_dec):
            below = encoder.output_channels[-(i + 1)]
            skipc = encoder.output_channels[-(i + 2)]
            stride = encoder.strides_for_stages[-(i + 1)]
...
class PlainConvUNet_modified(nn.Module):
    def __init__(self,
                 input_channels: int,
                 n_stages: int,
                 features_per_stage: Union[int, List[int], Tuple[int, ...]],
                 ...
                 nonlin_first: bool = False,
                 aspp_dilations: Tuple[int, ...] = (12,18,24)):
        super().__init__()
        ...
        #Encoder
        self.encoder = PlainConvEncoder_modified(
        ...
        #Decoder
        self.decoder = UNetDecoder_modified(
